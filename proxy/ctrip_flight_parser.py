...
def parse_inter_flights(data, trip_type) -> list:
    """解析国际航班列表
    :param data: 网络请求响应数据
    ...
    def minute2hour(minutes) -> str:
        ...
        if minutes < 60:
            ...
            return (
                "{hour}小时{min}分钟".format(hour=hour, min=left_minute)
                if left_minute > 0
                else "{hour}小时".format(hour=hour)
            )

    def parse_flight(segment, prices) -> dict:
        """航班信息解析
        :param segment: 去程/返程
        :param prices 产品价格信息
        :return: 航班信息
        ...
        for index, item in enumerate(prices):
            product = {
                ...
                "penalty_rule": None,  # 退改签规则
                "notes": item.get("purchaseNotes"),  # 备注
                "trip_type": trip_type,  # 行程类型  1: 单程  2: 去程  3: 返程
            }
    ...
    for itinerary in itineraries:
        ...
        if len(segments) == 1:
            # 单程/去程
            trip = parse_flight(segments[0], prices)
        else:
            # 返程
            if go_flight is None:
                go_flight = parse_flight(segments[0], prices)
            trip = parse_flight(segments[1], prices)
        trips.append(trip)
    return trips, go_flight


def parse_penalty_rule(rules):
    """解析退改签说明
    :param rules: 退改签信息列表
    """
    results = []
    for rule in rules:
        penalties = []
        for penalty in rule.get("penaltyList"):
            desc = ""
            fees = penalty.get("feeList")
            if fees:
                for fee in fees:
                    desc += (
                        fee.get("condition")
                ...
                desc = penalty.get("content")
            penalties.append({"label": penalty.get("label"), "desc": desc})
        results.append({"title": rule.get("title"), "penalties": penalties})
    return results
